...
const LIST_GROUP=document.querySelector(".mt-3");
...
function delete_item_from_str(params_todo){
...
function removeOrFind(e) {
  ...
  if (targetSpace.className !== 'form-check-input me-1') {
    if (targetSpace.className === 'bi bi-calendar-x' ) {
      targetSpace.parentElement.style.textDecoration="line-through"
      targetSpace.parentElement.remove();
      let deleted_item=e.target.parentElement.textContent.trim();
      console.log(deleted_item);
      delete_item_from_str(deleted_item);
